...
def view_thesis(request, thesis_id):
    ...
    if not thesis.file or not os.path.exists(thesis.file.path): raise Http404()
...
def serve_thesis_page_image(request, thesis_id, page_num):
    ...
    try:
        ...
        doc = fitz.open(thesis.file.path); page = doc.load_page(int(page_num) - 1)
        q = request.GET.get('q', '').strip()
        if q:
            for quad in page.search_for(q, quads=True):
                h = page.add_highlight_annot(quad); h.set_colors(stroke=[1, 1, 0]); h.update()
...
def view_thesis_file(request, pk):
    ...
    if request.user.profile.role not in [Profile.ADMIN, Profile.LIBRARIAN]: return HttpResponseForbidden()
...
def view_thesis_file_highlight(request, pk):
    ...
    if request.user.profile.role not in [Profile.ADMIN, Profile.LIBRARIAN]: return HttpResponseForbidden()
    q = request.GET.get('q', '').strip()
    if not q: return view_thesis_file(request, pk)
    doc = fitz.open(thesis.file.path)
    for p in doc:
...
def restricted_view_thesis_file(request, pk):
    ...
    for i in range(min(3, len(pdf_reader.pages))): pdf_writer.add_page(pdf_reader.pages[i])
    buf = io.BytesIO(); pdf_writer.write(buf); buf.seek(0)
    return HttpResponse(buf.getvalue(), content_type='application/pdf')
